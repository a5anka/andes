...
public class QpidAndesBridge {
     ...
     * @param sendConsumeOk A thread to send basic.consume_ok frame to consumer.
     * @throws AMQException
     */
    public static void createAMQPSubscription(Subscription subscription, AMQQueue queue, Runnable sendConsumeOk)
            throws AMQException {
        try {
            if (log.isDebugEnabled()) {
            ...
            if (subscription instanceof SubscriptionImpl.BrowserSubscription) {
                ...
                addLocalSubscriptionsForAllBindingsOfQueue(queue, subscription, sendConsumeOk);
            }
